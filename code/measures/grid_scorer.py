...
The code here automates the process 
...
## top level use-case function
def get_grid_scores(ratemaps, binx, biny):
    '''Computes a grid score for each ratemap.
    
    Vars:
    ----
    ratemaps: np.array #(n_neurons, n_bins_x, n_bins_y)
        firing rate (or occupancy) of neurons at each spatial location.
    binx: np.array #(n_bins_y+1)
        bin edges in metres
    biny: np.array #(n_bins_y+1)
        bin edges in metres
    
    Returns'''
    grid_scores = []
    sacs = []
    for ratemap in tqdm(ratemaps):
    ...
    return grid_scores, sacs
## Functions and grid scorer class 
