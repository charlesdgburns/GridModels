...
# here we want to generate hexagonal grid cells from path integration over place cell activations (velocity -> place cells; vel2pc)
...
- 3. Training loop and saving 

# NB: using Adam as a weight optimizer with ReLU activations induces sparsity on its own (https://arxiv.org/pdf/1812.08119)
#     here we use AdamW which decouples weight decay and we also apply Amsgrad.
"""
## setup ##
...
#local imports
from GridModels.code.measures import position_decoders as decoding
...
options.batch_size = 32      # number of trajectories per batch
options.sequence_length = 20  # number of steps in trajectory sub-sequence (for training)
options.n_epochs = 101       # number of epochs during training
...
options.Ng = 32**2             # number of grid cells / sorscher do 64**2 by default
...
options.optimizer = 'adam'   # 'sgd' or 'adam'
options.sparsity = 0.0        # strength of weight decay on recurrent weights
...
class vel2pc_RNN(torch.nn.Module):
    ...
    def compute_loss(self, outputs, targets):
        ...
        weight_loss = self.sparsity * torch.abs(self.rnn.weight_hh_l0).sum()
        # Total loss
...
def train_model(options, save = True):
    ...
    if options.optimizer == 'sgd':
        ...
        #NB: we want to avoid implicit sparsity in Adam (https://arxiv.org/pdf/1812.08119)
        # we can try doing so with amsgrad = True, or using AdamW with weight_decay = 0.
        # the latter seems more explicit, so let's go for that. (though amsgrad = true might be better gradient descent)
        optimizer = torch.optim.AdamW(model.parameters(), 
                                      lr = options.learning_rate, 
                                      weight_decay = 0,
                                      amsgrad=True)
        #note that we use AdamW with weight_decay = 0 as standard Adam induces implicit sparsity.
