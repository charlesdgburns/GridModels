...
Some code here is stolen from Tom George's RatInABox library and then modified to go fast on pytorch tensors
...
from GridModels.code.maze import representations as mr
from GridModels.code.maze import plotting as mp
...
def get_grid_cell_module_on_maze(n_neurons, scale_in_metres, orientation, bin_mask, metres_per_bin):
    '''For a mask indicating valid bins, returns a '''
...
def get_phase_from_params(params_dict):
    '''Takes dictionary with 'scale','orientation','offset' as inputs.
    Returns the offset's corresponding phase in toroidal space.'''
    ...
    phi=params_dict['orientation'] #global orientation of grid pattern
    ...
    centered_positions =  params_dict['offset']
