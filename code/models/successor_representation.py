...
def get_SR_from_bin_mask(bin_mask, discount_factor = 0.85):
    ...
    Bin mask must have TRUE/FALSE or 1/0 entries only.
    ...
    temp_matrix =  np.identity(T.shape[1])-discount_factor*T #(I - gammaT)^-1, so gamma here is discount factor defined above.
    SR_matrix = np.linalg.inv(temp_matrix) #Final output!
